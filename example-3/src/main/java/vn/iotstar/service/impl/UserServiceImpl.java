...
public class UserServiceImpl implements UserService {
    ...
    public void delete(Long id) {
        User user = userRepository.findById(id)
        .orElseThrow(() -> new IllegalArgumentException("User không tồn tại"));
        if (userRepository.countProductsByUserId(id) > 0)
        throw new IllegalArgumentException("Hãy xóa các sản phẩm của user trước khi xóa user");
    ...
    public UserServiceImpl(UserRepository userRepository, RoleRepository roleRepository, UserMapper mapper, PasswordEncoder passwordEncoder) {
